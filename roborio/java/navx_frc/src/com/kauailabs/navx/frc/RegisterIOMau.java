...
class RegisterIOMau implements IIOProvider {
    ...
    public RegisterIOMau( byte update_rate_hz, IIOCompleteNotification notify_sink, IBoardCapabilities board_capabilities  ) {
        ...
        try {
            ...
            System.out.println("Succesfully loaded vmxHaljni library.");
    ...
    private boolean getConfiguration() {
        ...
        while ( retry_count < 3 && !success ) {
 	    ...
 	    if (com.kauailabs.vmx.AHRSJNI.ReadConfigurationData(IMURegisters.NAVX_REG_WHOAMI, config, (byte)config.length)) {
	        System.out.println("Received AHRS Configuration Data:  " + config.length + " bytes.");
	    	...
	    	System.out.println("AHRSJNI.ReadConfigurationData return false.");
                success = false;
                Timer.delay(0.05);
            }
            retry_count++;
	...
	if (!success) {
	    System.out.println("Completely failed to received AHRS Configuration Data.");
	}
        return success;
    }
