...
VMXCAN::VMXCAN(CANClient& can_ref) :
	can(can_ref)
{
...
bool VMXCAN::GetCANBUSStatus(VMXCANBusStatus& bus_status, VMXErrorCode *errcode)
{
	...
	bus_status.busOffCount = 0; /* Todo:  Add this to VMX-pi CAN registers */
	bus_status.txFullCount = 0; /* Todo:  Add this to VMX-pi CAN registers */
					...
					for ( int i = 0; i < num_to_transfer; i++ ) {
						if (!rx_data[i].transfer.id.sidl.invalid) {
							...
							if (p_stream) {
								...
								printf("ERROR locating VMXCANReceiveStream for message id 0x%08x\n", messageID);
							}
						} else {
...
bool VMXCAN::CloseReceiveStream(VMXCANReceiveStreamHandle streamid, VMXErrorCode *errcode) {
	...
	uint8_t rxb_id = p_stream->GetRXBID();
	uint8_t rxb_filter_id = p_stream->GetRXBFilterID();
	uint8_t remaining_num_filters_in_use;
	if (p_stream_mgr->UnreserveFilter(rxb_id, rxb_filter_id, remaining_num_filters_in_use)) {
		CAN_ID unused_filter;
		VMXCANReceiveStream::CANMessageIDToCAN_ID(0xFFFFFFFF, unused_filter);
		if (!can.set_rxbuffer_accept_filter((CANClient::CANRXBufferID)rxb_id, rxb_filter_id, unused_filter)) {
			/* Log error, but continue */
			log_warning("Error clearing CAN Filter %d for RX Buffer %d.\n", rxb_filter_id, rxb_id);
		}
		if (remaining_num_filters_in_use) {
			CAN_ID unused_mask;
			VMXCANReceiveStream::CANMessageIDToCAN_ID(0xFFFFFFFF, unused_mask);
			if (!can.set_rxbuffer_accept_mask((CANClient::CANRXBufferID)rxb_id, unused_mask)) {
				/* Log error, but continue */
				log_warning("Error clearing CAN Mask for RX Buffer %d.\n", rxb_id);
			}
		}
		if (!p_stream_mgr->RemoveStream(streamid)) {
