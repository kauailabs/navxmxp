...
class AHRSInternal : public IIOCompleteNotification, public IBoardCapabilities {
    ...
    void SetAHRSPosData(AHRSProtocol::AHRSPosUpdate& ahrs_update, long sensor_timestamp) {
        ...
        for (int i = 0; i < MAX_NUM_CALLBACKS; i++) {
            ...
            if (callback != NULL) {
            	long system_timestamp = (long)(Timer::GetMonotonicTimestamp() * 1000);
...
AHRS::AHRS(SPIClient& client, PIGPIOClient& pigpio, uint8_t update_rate_hz)
{
    commonInit( update_rate_hz );
    io = new RegisterIO(new RegisterIO_SPI(client, pigpio), update_rate_hz, ahrs_internal, ahrs_internal);
