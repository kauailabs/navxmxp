...
PIGPIOClient::PIGPIOClient(IPIGPIOInterruptSink& interrupt_sink) :
	sink(interrupt_sink)
{
...
void PIGPIOClient::gpio_isr(int gpio, int level, uint32_t tick, void *userdata)
{
	...
	case vmx_pi_iocx_int_bcm_pin:
		p_this->sink.iocx_interrupt(level, tick);
		break;

	case vmx_pi_can_int_bcm_pin:
		p_this->sink.can_interrupt(level, tick);
		break;

	case vmx_pi_ahrs_int_bcm_pin:
		p_this->sink.ahrs_interrupt(level, tick);
		break;

	default:
		p_this->sink.pigpio_interrupt(gpio, level, tick);
		break;
	}
}
